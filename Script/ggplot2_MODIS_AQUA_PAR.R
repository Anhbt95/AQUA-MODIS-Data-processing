...
#MODIS AQUA Photosynthetically Available Radiation (PAR)
#Tuan-Anh Bui
...
(f <- list.files(path = paste0("C:/Users/TABU/Desktop/wget/MODIS_Aqua_Monthly_PAR_01.2006_04.2019","/"), pattern="*.L3m_MO_PAR_par_4km",full.names=T))
...
min(df_min_max$value_min, na.rm = T) #Min value - 20.2
max(df_min_max$value_max, na.rm = T) #Max value - 58.6
# Very large value of value_max compared to value_min 
# We do not need the log scale for the legend
...
#No special distribution in max value
...
legend_break <- seq(from = 20, to = 60, by = 5)
...
##Note - Change the save directory at the end of the loop script before running the loop
...
for (i in 1:length(year_list)) { 
  ...
  plot <- ggplot() +
    ...
    #Adjust lables
    labs(
      title = "MODIS AQUA Photosynthetically Available Radiation (PAR)", #Title 
    ...
    scale_fill_gradientn(
      name = "Photosynthetically Available Radiation (Einstein/mg².d)", #Title of the legend
  ...
  ggsave(path = paste0("C:/Users/TABU/Desktop/Tuan-Anh Bui/2_Deep sea/R_analysis/Figures/MODIS_AQUA_PAR_2006_2019","/"),
         filename = paste("AQUA_MODIS_PAR_",year_list[i],".png"), scale=2,
         plot = last_plot(),
  ...
  # ggsave(path = paste0("C:/Users/TABU/Desktop/Tuan-Anh Bui/2_Deep sea/R_analysis/Figures/MODIS_AQUA_PAR_2006_2019","/"),
  #      filename = paste("AQUA_MODIS_PAR_",year_list[i],".pdf"), scale=2,
  #      plot = last_plot(),
