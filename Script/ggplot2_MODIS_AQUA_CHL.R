...
#MODIS AQUA Chlorophyll_a concentration plot
#Tuan-Anh Bui
...
(f <- list.files(path = paste0("C:/Users/TABU/Desktop/wget/MODIS_Aqua_Monthly_CHL_chlor_a_01.2006_04.2019","/"), pattern="*.L3m_MO_CHL_chlor_a_4km.nc",full.names=T))
...
## Export data frame - even in a small area, the size of file in .csv format would be extremely large
...
min(df_min_max$value_min, na.rm = T) #Min value - 0.015
max(df_min_max$value_max, na.rm = T) #Max value - 32.23

#Very large value of value_max chlorophyll a
#Use frequency plot to explore the value_max distribution
ggplot(df_min_max) + geom_freqpoly(aes(value_max), binwidth = 0.1)
...
ggplot(df_min_max) + geom_freqpoly(aes(value_max), binwidth = 0.1) + coord_cartesian(xlim = c(0,5))
#Most of values are under 2

#Set logarith break of the legend
#Set break from 0.01 to 20 as NASA 
#legend_break <- c(0.01,0.02,0.05,0.1,0.2,0.5,1,2,5,10,20)
#Set logarith break from 0.01 to 2 
legend_break <- c(0.01,0.02,0.05,0.1,0.2,0.5,1,2)

#Create set of colors 
#Use the reversed Spectral color palette (Blue - Red) for the logarith range 0.01-20 as NASA
#RLcolor <- rev(brewer.pal(11, "Spectral")) %>%
#  colorRampPalette(bias=1,space="Lab",interpolate="linear")

#Use the shade from purple to red for the logarith range 0.01-2
RLcolor <- colorRampPalette(c("purple","darkblue","royalblue", "skyblue","green","yellow","red"),bias=1,space="Lab",interpolate="linear")
...
for (i in 1:length(year_list)) { 
  ...
  plot <- ggplot() + 
    ...
    #Adjust lables
    labs(
      title = "MODIS AQUA Chlorophyll a concentration (mg/m³)", #Title 
    ...
    scale_fill_gradientn(
      name = "Chlorophyll a concentration (mg/m³)", #Title of the legend
      ...
      #The values out of limit range will 
  ...
  ggsave(path = paste0("C:/Users/TABU/Desktop/Tuan-Anh Bui/2_Deep sea/R_analysis/Figures/MODIS_AQUA_CHL_2006_2019","/"),
         filename = paste("AQUA_MODIS_CHL_",year_list[i],".png"), scale=2,
         plot = last_plot(),
  ...
  # ggsave(path = paste0("C:/Users/TABU/Desktop/Tuan-Anh Bui/2_Deep sea/R_analysis/Figures/MODIS_AQUA_CHL_2006_2019","/"),
  #       filename = paste("AQUA_MODIS_CHL_",year_list[i],".pdf"), scale=2,
  #      plot = last_plot(),
