...
(f <- list.files(path = paste0("C:/Users/TABU/Desktop/wget/MODIS_Aqua_Monthly_PIC_01.2006_04.2019","/"), pattern="*.L3m_MO_PIC_pic_4km.nc",full.names=T))
...
min(df_min_max$value_min, na.rm = T) #Min value - 1.2e-5
max(df_min_max$value_max, na.rm = T) #Max value - 0.0024
...
ggplot(df_min_max) + geom_freqpoly(aes(value_min), binwidth = 0.0000001)
#Use frequency plot to explore the value_max distribution
ggplot(df_min_max) + geom_freqpoly(aes(value_max), binwidth = 0.0000001)
...
legend_break <- c(1e-05, 2e-04, 5e-05, 1e-04, 2e-04, 5e-04, 1e-03, 2e-03, 3e-03)
...
for (i in 1:length(year_list)) { 
  ...
  plot <- ggplot() +
    ...
    #Adjust lables
    labs(
      title = "MODIS AQUA Particulate Inorganic Carbon (PIC)", #Title 
    ...
    scale_fill_gradientn(
      name = "Particulate Inorganic Carbon (mol/m³)", #Title of the legend
  ...
  ggsave(path = paste0("C:/Users/TABU/Desktop/Tuan-Anh Bui/2_Deep sea/R_analysis/Figures/MODIS_AQUA_PIC_2006_2019","/"),
         filename = paste("AQUA_MODIS_PIC_",year_list[i],".png"), scale=2,
         plot = last_plot(),
  ...
  #ggsave(path = paste0("C:/Users/TABU/Desktop/Tuan-Anh Bui/2_Deep sea/R_analysis/Figures/MODIS_AQUA_PIC_2006_2019","/"),
  #       filename = paste("AQUA_MODIS_PIC_",year_list[i],".pdf"), scale=2,
  #      plot = last_plot(),
